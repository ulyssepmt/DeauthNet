...
# ========================================== CONF
def setchan(
        chan=13):  # la fonction suivante permet d'initialiser la carte réseau de la machine actuelle en mode monitor selon le système sur lequel s'exécute le programme
    global iface
    if SYSTEM == "Linux":
...
def onClick(): # Changement d'Etat du boutton "Start Deauth attack"
    global button_state
    if check_device():
        if button_state == "Start Deauth attack":
            ...
            if usrchan.get().isnumeric() and 0 < int(usrchan.get()) < 150:
                ...
                messagebox.showerror(title,
                                     f"Unable to start the attack on the channel {usrchan.get()} !\nPlease change channel (1-149)")
    else:
...
def aboutButton():
    messagebox.showinfo(title,
                        "Program created by Ulysse & Jean-Baptiste during the SAE 304 in networking at the IUT of Saint-Malo (Feb 2023).\n\nHow to use it:\n1st: Refresh the list ↻\n\n2nd: Select a channel for the targeted AP.\n\n3rd: Start deauthentication attack !")
...
frconf = LabelFrame(root, text="System Configuration", fg='red', bg='black', font="Monaco 12 bold", width=130,
                    height=50)

Deauthtxt = Label(frconf, text=f"OS Detected: {SYSTEM}", font="Monaco 10 bold", foreground='#7F00FF',
                  background="#000000")
...
Button(root, text="About", font="Monaco 10 bold", fg="white", bg="#525252", height=1, command=aboutButton).place(x=10,
                                                                                                                 y=510)
# Labels SSID avec leurs MACs spécifiés
button_state = "no"
btndn = Button(root, text="Start Deauth attack", font="Monaco 12 bold", fg="#22FF00", bg="#525252", height=2,
               command=onClick)
